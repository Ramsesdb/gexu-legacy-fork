...
class HistoryScreenModelStateProvider : PreviewParameterProvider<HistoryScreenModel.State> {
    ...
    private val loadingWithSearchQuery =
        HistoryScreenModel.State(
    ...
    override val values: Sequence<HistoryScreenModel.State> =
        sequenceOf(
            multiPage,
            shortRecent,
            shortFuture,
    ...
    private object HistoryUiModelExamples {
        ...
        fun header(instantBuilder: (Instant) -> Instant = { it }) = HistoryUiModel.Header(LocalDate.from(instantBuilder(Instant.now())))
