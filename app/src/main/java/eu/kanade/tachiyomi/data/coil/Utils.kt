...
internal inline fun Size.widthPx(
    scale: Scale,
    original: () -> Int,
): Int = if (isOriginal) original() else width.toPx(scale)

internal inline fun Size.heightPx(
    scale: Scale,
    original: () -> Int,
): Int = if (isOriginal) original() else height.toPx(scale)

internal fun Dimension.toPx(scale: Scale): Int =
    pxOrElse {
        when (scale) {
...
fun ImageRequest.Builder.cropBorders(enable: Boolean) =
    apply {
...
fun ImageRequest.Builder.customDecoder(enable: Boolean) =
    apply {
